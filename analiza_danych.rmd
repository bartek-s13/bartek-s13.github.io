...
p <- ggplot(gold_prices, aes(x=date, y=value, color=waluta))  + geom_line() + labs(x="Data", y="Wartość w odpowiedniej walucie", title="Interaktywny wykres ceny złota w róznych walutach") + 
...
Ze względu na późniejsza próbę przewidywania cen złota będe szukane jedeynie korelacje pomiędzy ceną złota w dolarach i wartościami innych wskaźników.

## Korelacje pomiędzy ceną złota w dolarach i wkażnikami rozwoju 

### Świat

Korelacje pomiędzy ceną złota a wskaźnikami rozoju dla świata, dla których wartość bezwzględna jest wieksza niż 0.9:
...
for (i in indicators) {
      ...
      if (nrow(joined) >= 30) {
        ...
        txt <- paste("Warość wpółczynika korelacji Pearsona", i , "do  ceny zlota wynosi", round(c, 2), "\n\n")
...
Ponieważ wskaźniki rozwoju określane są z dokładnością do roku to przy obliczaniu korelacji wzięta została średnia cena złota w danym roku. Dodatkowo pod uwagę zostały wzięte tylko takie pary cen złota i wartości wskaźnika, które istnieją dla przynajmniej 30 lat.
...
Korelacje pomiędzy ceną złota a wskaźnikami rozoju dla Stanów Zjedoczonych, dla których wartość bezwzględna jest wieksza niż 0.9:
...
Podobnie jak miało to miejsce w przypadku obliczania korelacji cen złota ze wskaźnikami rozwoju dla świata użyte zostały średnie ceny złota w danym roku. Również wykorzystano zostały jedynie te pary wskaźnik-cena złota, dla których istieją dane z co najmniej 30 lat.
...
Korelacje pomiędzy ceną złota a wskaźnikami z S P Composite, dla których wartość bezwzględna jest wieksza niż 0.75 (w zbiorze nie ma tak silnych korelacji ja w zbiorze ze wskaźnikami rozwoju):
...
```
Ponieważ wskaźniki ze zbioru S P Composite są wyznaczone z dokładnością co do miesiąca to do obliczenia korelacji została wykorzystana średnia cena złota dla danego miesiąca.
...
for (i in currencies) {
      ...
      joined <- inner_join(x, zloto_dolary, by="date")
        c <- cor(joined$value.x, joined$value.y)
        txt <- paste("Korelacja kursu Dolar amerykański/", i , " do  ceny zlota wynosi ", round(c, 2), "\n\n", sep="")
        if (!is.na(c) & abs(c) > 0.75){
          cat(txt)
...
Korelacje ceny złota i wskaźników dotyczących Bitocina są małe. Dodatkowo dane dotyczące Bitcoina pochodzą z krótkiego okresu czasu w prówaniu do wskaźników z innych zbiorów oraz do cen złota. Z tego powodu nie zostaną one wzięte pod uwagę przy próbieprzewidywania cen złota.
